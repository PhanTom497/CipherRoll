...
export class CipherRollDatabase {
  ...
  private async migrate() {
    await this.query(`
      ...
      CREATE TABLE IF NOT EXISTS organizations (
        ...
        reserved_admin_slots BIGINT NOT NULL,
        reserved_quorum BIGINT NOT NULL,
      ...
      CREATE TABLE IF NOT EXISTS organization_insights (
        org_id TEXT PRIMARY KEY,
        total_payroll_items BIGINT NOT NULL,
        active_payroll_items BIGINT NOT NULL,
        claimed_payroll_items BIGINT NOT NULL,
        vesting_payroll_items BIGINT NOT NULL,
        employee_recipients BIGINT NOT NULL,
      ...
      CREATE TABLE IF NOT EXISTS payroll_runs (
        ...
        planned_headcount BIGINT NOT NULL,
        allocation_count BIGINT NOT NULL,
        claimed_count BIGINT NOT NULL,
        created_at BIGINT NOT NULL,
  ...
  async upsertOrganization(record: OrganizationRecord) {
    await this.query(
      `
  ...
  async upsertOrganizationInsights(record: OrganizationInsightsRecord) {
    await this.query(
      `
  ...
  async upsertTreasuryRoute(record: TreasuryRouteRecord) {
    await this.query(
      `
  ...
  async upsertPayrollRun(record: PayrollRunRecord) {
    await this.query(
      `
  ...
  async upsertAuditReceipt(record: AuditReceiptRecord) {
    await this.query(
      `
  ...
  async insertRawEvent(record: RawEventRecord) {
    await this.query(
      `
  ...
  async insertNotification(record: NotificationRecord) {
    await this.query(
      `
  ...
  async getPayrollRunsForOrganization(orgId: string, options?: { status?: number; limit?: number }) {
  ...
  async getPayrollRun(payrollRunId: string) {
    const result = await this.query("SELECT * FROM payroll_runs WHERE payroll_run_id = $1", [
      payrollRunId
    ]);
    return result.rows[0];
  }
  ...
  async getPayment(paymentId: string) {
    const result = await this.query("SELECT * FROM payments WHERE payment_id = $1", [paymentId]);
    return result.rows[0];
  }

  async getPaymentsForOrganization(
    orgId: string,
    limit = 100,
    options?: {
  ...
  async getAuditReceipts(
    ...
    if (typeof options?.published === "boolean") {
      ...
      clauses.push(`published = $${params.length}`);
    }

    if (options?.receiptKind) {
      params.push(options.receiptKind);
      clauses.push(`receipt_kind = $${params.length}`);
    }
  ...
  async getRawEvents(orgId?: string, eventName?: string, limit = 100) {
    const clauses: string[] = [];
    const params: SqlParam[] = [];
  ...
  async getOrganizationReportSummary(orgId: string): Promise<OrganizationReportSummary | null> {
  ...
  async getOrganizationAuditPackage(orgId: string): Promise<OrganizationAuditPackage | null> {
    ...
    const recentAuditReceipts = ((await this.getAuditReceipts(orgId, 25)) as Record<string, unknown>[]).map(
      (row) => rowToJson(coerceRowBooleans(row, ["published"])) as unknown as AuditReceiptRecord
    );
    const recentNotifications = ((await this.getNotifications(orgId, 25)) as Record<string, unknown>[]).map(
      (row) => rowToJson(row) as unknown as NotificationRecord
    );
  ...
  async getOrganizationExportPackage(orgId: string): Promise<OrganizationExportPackage | null> {
    ...
    const auditReceipts = ((await this.getAuditReceipts(orgId, 250)) as Record<string, unknown>[]).map(
      (row) => rowToJson(coerceRowBooleans(row, ["published"])) as unknown as AuditReceiptRecord
    );
    const notifications = ((await this.getNotifications(orgId, 250)) as Record<string, unknown>[]).map(
      (row) => rowToJson(row) as unknown as NotificationRecord
    );
  ...
  async getIndexerStatus(): Promise<IndexerStatus> {
    const latestIndexedBlock = Number((await this.getMetadata("indexer.latestIndexedBlock")) || "0");
    const latestKnownBlock = Number((await this.getMetadata("indexer.latestKnownBlock")) || "0");
    const lastSyncStartedAt = await this.getMetadata("indexer.lastSyncStartedAt");
    const lastSyncFinishedAt = await this.getMetadata("indexer.lastSyncFinishedAt");
    const lastSyncError = await this.getMetadata("indexer.lastSyncError");

    const countsResult = await this.query<{
      organizations: string | number;
      payroll_runs: string | number;
      payments: string | number;
      audit_receipts: string | number;
      raw_events: string | number;
