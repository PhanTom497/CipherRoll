...
export default function AuditorPage() {
  ...
  const reviewChecklist = [
    { label: 'Auditor wallet connected', complete: Boolean(address) },
    { label: `${TARGET_CHAIN_NAME} selected`, complete: isTargetChain },
    { label: 'Auditor access enabled', complete: cofheReady },
    { label: 'Recipient permit imported', complete: recipientPermits.length > 0 },
    { label: 'Active recipient permit selected', complete: Boolean(activePermit) },
    { label: 'Workspace aggregate summary loaded', complete: summary.totalPayrollRuns > 0 || Boolean(summaryValues.budget) },
    { label: 'Provable audit evidence generated', complete: Boolean(lastEvidenceReceipt || lastBatchEvidenceReceipt) }
  ]

  const analyticsCards = [
    { label: 'Budget', value: summaryValues.budget ?? '***', detail: 'Encrypted organization budget decrypted through the shared permit.' },
    { label: 'Committed Payroll', value: summaryValues.committed ?? '***', detail: 'Aggregate payroll commitments, never employee-level rows.' },
    { label: 'Available Runway', value: summaryValues.available ?? '***', detail: 'Aggregate available runway after commitments.' },
    { label: 'Payroll Runs', value: String(summary.totalPayrollRuns), detail: `${summary.draftPayrollRuns} draft, ${summary.fundedPayrollRuns} funded, ${summary.activePayrollRuns} active.` },
    { label: 'Employee Recipients', value: String(summary.employeeRecipients), detail: 'Unique employee wallets touched by this organization.' },
    { label: 'Treasury Status', value: summary.treasuryRouteConfigured ? treasuryAvailable : 'No route', detail: summary.treasuryRouteConfigured ? `${treasuryReserved} reserved` : 'Treasury-backed settlement is not configured.' }
  ]

  const currentAuditModeLabel = lastBatchEvidenceReceipt
    ? `${lastBatchEvidenceReceipt.mode === 'verify' ? 'Batched verified' : 'Batched published'} receipt`
    : lastEvidenceReceipt
      ? `${lastEvidenceReceipt.mode === 'verify' ? 'Verified' : 'Published'} single-metric receipt`
      : summaryValues.budget || summaryValues.committed || summaryValues.available
        ? 'View-only permit review'
        : 'No disclosure active yet'
      ...
      <div className="w-full max-w-6xl mx-auto px-6 pb-24 relative z-10">
        <section className="mb-8 text-center">
          ...
          <p className="text-[#a1a1aa] text-lg md:text-xl max-w-3xl mx-auto leading-relaxed">
            Review aggregate-only payroll disclosures on {SUPPORTED_CHAIN_NAMES}. CipherRoll keeps employee-level salary rows off this surface and decrypts only the organization summaries that an admin explicitly shared.
          </p>
        </section>
        ...
        <div className="mt-8 grid gap-4">
          ...
          <GlassCard className="p-6 border-white/5 bg-[#0a0a0a] rounded-3xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Disclosure mode</h2>
                <p className="text-sm text-[#a1a1aa] mt-1">
                  CipherRoll separates private review from on-chain evidence so auditors can understand exactly when they are only viewing shared data versus when they are creating a permanent receipt.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 p-5">
                <p className="text-xs uppercase tracking-[0.18em] text-cyan-100/80 font-bold">Current mode</p>
                <p className="mt-3 text-2xl font-black text-white">{currentAuditModeLabel}</p>
                <p className="mt-3 text-sm text-cyan-50/90">
                  View-only review keeps plaintext local to this browser session. Receipt mode adds an on-chain record for the selected aggregate metric or batch.
                </p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                <p className="text-xs uppercase tracking-[0.18em] text-white/55 font-bold">View-only permit review</p>
                <p className="mt-3 text-sm text-[#c9c9d0]">
                  Uses <span className="font-mono text-white">decryptForView(...)</span> with the active recipient permit. The decrypted aggregate values stay local to the auditor browser and do not create a receipt on-chain.
                </p>
              </div>
              <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-5">
                <p className="text-xs uppercase tracking-[0.18em] text-emerald-50/80 font-bold">Provable receipt mode</p>
                <p className="mt-3 text-sm text-emerald-50/90">
                  Uses <span className="font-mono text-white">decryptForTx(...)</span>. Verify creates a narrow on-chain receipt; publish also makes the decrypt result available to downstream contract consumers.
                </p>
              </div>
            </div>
          </GlassCard>
          ...
          <div className="space-y-6">
            <GlassCard className="p-8 border-white/5 bg-[#0a0a0a] rounded-3xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Import shared payload</h2>
                  <p className="text-sm text-[#a1a1aa] mt-2">
                    Paste the exported sharing payload from the admin portal. The current SDK will import it, sign it for this wallet, and store it as a recipient permit.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <textarea
                  ...
                  className="min-h-[220px] w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-mono text-white/85 placeholder:text-white/35"
                  placeholder="Paste the non-sensitive sharing payload from the admin portal here."
                />
                <button
              ...
              <div className="mt-6 rounded-2xl border border-cyan-400/15 bg-cyan-400/10 p-4 text-sm text-cyan-50">
                The imported permit only unlocks the aggregate budget summary handles that the admin chose to share. It does not grant access to employee payroll rows.
              </div>
              <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-[#c9c9d0]">
                ...
                <p className="mt-2">
                  The imported recipient permit only works because the data owner previously granted on-chain <span className="font-mono text-white">FHE.allow(...)</span> access to the aggregate handles that Goal 1 exposed. CipherRoll does not reuse admin-only salary getters here.
                </p>
              </div>
            </GlassCard>
            ...
            <GlassCard className="p-8 border-white/5 bg-[#0a0a0a] rounded-3xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#a1a1aa] mt-2">
                    Choose which imported recipient permit is active before decrypting the aggregate summary handles.
                  </p>
                </div>
              </div>
                      ...
                      >
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                          <div>
                            ...
                            <p className="mt-2 text-sm text-[#c9c9d0]">Issuer: <span className="font-mono text-white/85">{permit.issuer}</span></p>
                            <p className="mt-1 text-sm text-[#c9c9d0]">Recipient: <span className="font-mono text-white/85">{permit.recipient}</span></p>
              ...
              <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm text-amber-50">
                Removing a recipient permit here stops local decrypts in this browser session. It does not retroactively cancel any other imported copy or override the permit expiration already signed by the admin and auditor.
              </div>
            </GlassCard>
          </div>
          ...
          <div className="space-y-6">
            <GlassCard className="p-8 border-white/5 bg-[#0a0a0a] rounded-3xl">
              <div className="flex items-center gap-3 mb-6">
                <Eye className="w-5 h-5 text-cyan-300" />
                <div>
                  <h2 className="text-2xl font-bold text-white">View-only permit review</h2>
                  <p className="text-sm text-[#a1a1aa] mt-2">
                    This surface centers on organization-level balances, commitments, counts, and policy-safe summaries. It intentionally avoids employee-level payroll history and does not create an on-chain receipt by itself.
                  </p>
                </div>
              </div>
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  <p className="text-xs uppercase tracking-[0.18em] text-white/55 font-bold mb-3">Runway / solvency status</p>
                  <p className="text-2xl font-black text-white">{solvencyLabel}</p>
                  <p className="mt-2 text-sm text-[#c9c9d0]">
                    {summaryValues.available
                      ? `${runwayPercent.toFixed(0)}% of the encrypted budget remains available after commitments.`
                      : 'Import a shared permit and refresh to decrypt the aggregate runway values.'}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  <p className="text-xs uppercase tracking-[0.18em] text-white/55 font-bold mb-3">Funding policy snapshot</p>
                  <p className="text-sm text-white">{summary.treasuryRouteConfigured ? 'Treasury route configured' : 'No treasury route configured'}</p>
                  <p className="mt-2 text-sm text-white">
                    {summary.supportsConfidentialSettlement
                      ? 'Confidential wrapper settlement is enabled.'
                      : 'Direct treasury settlement or no confidential wrapper is configured.'}
                  </p>
                </div>
              </div>
            </GlassCard>
            ...
            <GlassCard className="p-8 border-white/5 bg-[#0a0a0a] rounded-3xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Compliance-safe summary</h2>
                  <p className="text-sm text-[#a1a1aa] mt-2">
                    Public organization data that stays useful for audits without revealing employee salary rows or raw allocation handles.
                  </p>
                </div>
              </div>
            ...
            <GlassCard className="p-8 border-white/5 bg-[#0a0a0a] rounded-3xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Provable audit receipt</h2>
                  <p className="text-sm text-[#a1a1aa] mt-2">
                    Move a shared aggregate, or a selected batch of shared aggregates, from view-only review to provable on-chain evidence. CipherRoll uses <span className="font-mono text-white">decryptForTx</span> with the active recipient permit, then verifies or publishes the result on-chain.
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-4">
                  ...
                  {(Object.entries(evidenceMetricDetails) as Array<[AuditorAggregateDisclosureMetric, (typeof evidenceMetricDetails)[AuditorAggregateDisclosureMetric]]>).map(([metric, meta]) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="mt-3 text-sm text-[#a1a1aa]">{meta.detail}</p>
                      </button>
                    )
                  })}
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-sm text-[#c9c9d0]">
                  ...
                  <p className="mt-2">
                    {selectedEvidenceValue
                      ? `Current shared value: ${selectedEvidenceValue}. Verify mode emits a narrow on-chain receipt. Publish mode also registers the decrypt result on the FHE task manager for downstream contract consumers.`
                      : 'Refresh the aggregate summary first so CipherRoll can generate evidence for this metric.'}
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-5 text-sm text-[#c9c9d0]">
                  ...
                  <p className="mt-2">
                    Select the aggregate metrics to include in one signed compliance batch. CipherRoll keeps the batch limited to shared organization summaries rather than employee-level encrypted state.
                  </p>
                ...
                <div className="rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm text-amber-50">
                  Verify mode is the safer default because it keeps the disclosure narrow. Publish mode is only for cases where later on-chain consumers must rely on the decrypted result directly.
                </div>
            ...
            <GlassCard className="p-8 border-white/5 bg-[#0a0a0a] rounded-3xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Review checklist</h2>
                  <p className="text-sm text-[#a1a1aa] mt-2">
                    CipherRoll keeps the audit review flow narrow: only wallet identity, recipient permit status, and aggregate summaries are required.
                  </p>
                </div>
              </div>
        ...
        <div className="mt-8">
          <GlassCard className="p-6 border-white/5 bg-[#0a0a0a] rounded-3xl">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 className="text-xl font-bold text-white">Privacy boundary</h3>
                <p className="mt-2 text-sm text-[#c9c9d0] max-w-4xl">
                  This portal decrypts only aggregate budget, committed payroll, and available runway values through the active recipient permit. Employee salary rows, per-payment allocation handles, and unnecessary PII are intentionally left out of this auditor surface.
                </p>
                <p className="mt-2 text-sm text-[#c9c9d0] max-w-4xl">
                  View-only permit review keeps plaintext local to the auditor browser. Provable receipt mode adds an on-chain record for the selected metric or batch, and publish mode goes one step further by making the decrypt result available for downstream contract use.
                </p>
                <p className="mt-2 text-sm text-[#c9c9d0] max-w-4xl">
                  Shared permits are short-lived and scoped by design, but product-level removal remains local to the wallet/browser storing the permit. Honest review means treating expiration and narrow scope as the main control, not implying a universal remote revoke.
                </p>
              </div>
              <Link href="/docs" className="inline-flex items-center gap-2 text-sm font-semibold text-white underline underline-offset-4">
                Read the disclosure notes
